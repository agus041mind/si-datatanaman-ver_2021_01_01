                ...
                <table class="table table-bordered table-striped table-sm" id="tableSindata" width="100%" cellspacing="0" style="color:#000000">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      @if($jmlVak > 1)
                        @if($item->indek == Sindata::indekVak('min'))
                          <button type="button" class="btn btn-sm btn-primary" onClick="window.location.href='{{route('adminVakTurun',$item->idVak)}}'"><i class='fas fa-caret-square-down'></i></button>
                        @elseif($item->indek == Sindata::indekVak('mak'))
                          <button type="button" class="btn btn-sm btn-primary" onClick="window.location.href='{{route('adminVakNaik',$item->idVak)}}'"><i class='fas fa-caret-square-up'></i></button>
                        @else
                          <button type="button" class="btn btn-sm btn-primary" onClick="window.location.href='{{route('adminVakTurun',$item->idVak)}}'"><i class='fas fa-caret-square-down'></i></button>
                          <button type="button" class="btn btn-sm btn-primary" onClick="window.location.href='{{route('adminVakNaik',$item->idVak)}}'"><i class='fas fa-caret-square-up'></i></button>                        
                        @endif
                      ...
                      <td class="text-center">
                        <a href="{{route('adminVakTampil',$item->idVak)}}">
                      ...
                      <td class="text-center">
                        <a href="{{route('adminVakHapus',$item->idVak)}}">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <b>Vak</b><br>
        {{$item->nama}}<p><p>
      <b>Lingkungan</b><br>
        {{$item->lingkungan->nama}}<p><p>
      <b>Wilayah</b><br>
        {{$item->lingkungan->wilayah->nama}}<p><p>
      <b>Perawat</b><br>
        {{$item->perawat}}<p>
      <b>Keterangan</b><br>
        {{$item->keterangan}}
