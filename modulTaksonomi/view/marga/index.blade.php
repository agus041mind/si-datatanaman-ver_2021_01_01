              ...
              <div class="table-responsive-xl">
                <table class="table table-bordered table-striped table-sm" id="tableSindata" width="100%" cellspacing="0" style="color:#000000">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{$item->nama}}</td>
                      <td>{{$item->suku->nama}}</td>
                      <td class="text-center">
                        <a href="{{route('adminMargaTampil',$item->idMarga)}}">
                      ...
                      <td class="text-center">
                        <a href="{{route('adminMargaHapus',$item->idMarga)}}">
						              <button type="button" class="btn btn-sm btn-danger" onClick="return confirm('Yakin mau dihapus?')" 
                          {{(($item->detailPenerimaan->count() == null || $item->detailPenerimaan->count() == 0) && ($item->ubahNama->count() == null || $item->ubahNama->count() == 0) && ($item->herbariumJenis->count() == null || $item->herbariumJenis->count() == 0)) ? "" : "disabled"}}
                          >
